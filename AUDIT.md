...
### [F-047] `applyMigrations` gained an optional `onWarning` parameter and can now throw where it previously applied — status: done (pending commit) — severity: med — area: api
...
- **Assessed real, and fixed**: `diffSnapshots`'s renamed-table step (`diff.ts`) already emitted the rename, the old-name guard drop, and (when the table stays append-only) the new-name guard re-create as three adjacent statements — but `statementGroups` (`sql.ts`) had no matching branch to recognise them as one group; each was its own singleton. **Fix**: `statementGroups` now recognises `alter table X rename to Y` followed by `drop trigger if exists "X_no_update"` (optionally followed by `create trigger "Y_no_update" ...`) as one indivisible group, mirroring the rebuild-tail extension `[F-041]` made. **Bonus catch**: while adding coverage, found and fixed a real, separate bug in the (already-present) `buildCreateTriggerPattern` helper — its trailing `\b` after the quoted-identifier alternation never matched the kit's own double-quoted spelling (a `\b` after `"` requires a word char on both sides, which a quote mark never has), so the three-statement group silently degraded to two whenever the table stayed append-only. Caught by the pre-existing `kit/test/unit/apply.test.ts` test for this shape, which was red before the regex fix and green after — that failure-then-pass *is* this fix's revert-verification.
