...
### [F-119] `isRowidAlias` (`kit/src/core/introspect.ts`) tests only the declared type, so `INTEGER PRIMARY KEY DESC` is snapshotted `notNull: true` incorrectly — status: todo — severity: low — area: kit/introspect
...
### [F-126] `guardChanged` credited a rename `columnRenames` named but the rename loop never emitted — status: **done** (this batch) — severity: **high**, regression vs the merge base — area: kit/diff
...
### [F-127] Four prototype-unsafe table-name maps survived round 8's sweep — status: **done** (this batch) — severity: **high** — area: kit/diff, kit/cli, kit/snapshot, kit/node/store
...
### [F-128] Missing regression: a pure 1:1 guarded-column rename must NOT restate the guard — status: **done** (this batch, test added) — severity: n/a (test-only) — area: kit/diff test coverage
...
### [F-129] Recorded, not fixed, per the reviewer's explicit instruction (three pre-existing findings)
