...
After the kit/node finish-and-land pass (2026-08-18, this session — landed the batch left uncommitted by a previous agent, plus the items it left undone): **green, 1010 passed / 7 skipped** (`npm run check`, full gate). `[F-115]`, `[F-100]` (the parked `tableOptions` sidecar half), `[F-110]`, `[F-047]` (**since reopened as `needs-human`** — it was "closed" by a comment asserting a contract nobody decided), `[F-038]`, `[F-065]`, `[F-066]` and `[F-116-cli]` (`push` asking `runner.atomicLimit`) were already done or finished in the pre-existing diff; this pass fixed the `URL`/`URL` typecheck failure blocking `npm run check` (`kit/test/unit/node-loading.test.ts`, a lib/dom-vs-node type mismatch — replaced `new URL(spec, import.meta.url)` with `join(dirname(fileURLToPath(import.meta.url)), spec)`), refuted `[F-075]` with evidence (`remoteRunner.batch` already routes any batch containing a trigger body through the file-import endpoint instead of re-splitting it — `kit/src/node/runners.ts`'s `viaImport`/`needsImport`, landed on `main` before this session in `977e83f`), added `push`'s missing `runner.atomicLimit` wiring (`kit/src/node/commands.ts`, mirroring `applyMigration`) with a new load-bearing unit test (`kit/test/unit/push-atomic-limit.test.ts`), and assessed-and-fixed `[F-117]` (new): a renamed append-only table's guard drop-and-restore was three statements a batch boundary could separate — `kit/src/core/diff.ts`'s renamed-table step now emits the new-name guard re-create immediately after the old-name drop (not deferred to a later, distant per-table pass), and `kit/src/core/sql.ts`'s `statementGroups` now recognises `rename` + `drop trigger if exists` (+ optional `create trigger`) as one indivisible group, the same way it already does for a rebuild's own tail. Every fix in this pass was verified load-bearing by reverting the production change and confirming its test goes red — **except `[F-117]`**, whose claimed verification is corrected in its own entry below: it never happened. `dist/core.js` is unchanged at 43,077 bytes — nothing in this pass touches `src/core.ts`'s entry graph; `src/ddl.ts`'s `TableOptions.collate` addition is in `orm-d1/ddl`, a separate entry point not bundled into `core.js`.
After the round-5 rejection follow-up (2026-08-18, this session): **green, `npm run check` exits 0** (`test:unit` + `test:workers`, full gate). Three findings fixed, one rejection (a rename that reuses a guarded column name silently un-guards it) confirmed and closed. `kit/src/core/diff.ts`'s `liveGuardMismatch` check used to catch only one direction of the collision — two `after`-guarded names collapsing onto one live name; it missed the other direction, where an `after`-guarded column's own name is the SOURCE of a rename this same diff performs (the expand/contract retype pattern), which silently carries the guard onto the column being renamed AWAY instead of the one that ends up holding the name. Replaced with the predicate the reviewer specified: forward-map the guard's live column set through `renamedColumns` and compare as sets against `finalGuarded`, which catches both directions; `liveGuardedActual` (what step 4 restates against) also had to change from the pre-rename live names to the forward-mapped set, or the SOURCE-collision case compared equal to `after`'s list and was never corrected. `kit/src/node/commands.ts`'s `renderTableOptionsModule` rendered a live `appendOnly` column list verbatim with no existence check — `appendOnlyTriggerGuard` (`core/introspect.ts`) reads a trigger's `UPDATE OF` list back exactly as SQLite stores it, with no check that the named column still exists (SQLite itself never enforces one), so a stale/foreign trigger naming a dropped or renamed column produced a sidecar `assertAppendOnlyColumns` (`src/ddl.ts`) then rejected outright on the next `generate`/`check`, out of a `pull` that had reported success — the collate branch right next to it already did this existence check, appendOnly did not. Now filtered the same way, with the drop reported through `unexpressibleTableOptionWarnings` rather than silently. And `snapshotFromSchema`'s (`kit/src/core/snapshot.ts:284`) and `introspect`'s (`kit/src/core/introspect.ts:618`) per-table column maps were still plain `{}` — the exact hazard every constraint map in the same functions is already `Object.create(null)` for, with a comment spelling it out, just missed for the column map itself: a column literally named `__proto__` (legal SQL, legal in the schema DSL) assigned through a plain object sets the prototype instead of adding an entry, so the column silently vanishes from the snapshot with no error, both producers drop it identically so a diff between them never notices, and a rebuild for any unrelated reason drops the column and its data. Fixed both, plus `tableOptions()`'s `byTable` map (`src/ddl.ts`) for the same reason — its existing `Object.hasOwn` guard stops a table named `constructor`/`toString`/… from reading a truthy inherited member, but a table named `__proto__` never becomes an own property at all under a plain object, so two declarations for it never tripped "declared twice" — and `diff.ts`'s two `columnRenames` map constructions, discovered as a direct consequence while writing this fix's own load-bearing test (`diffSnapshots` of an unchanged schema with a `__proto__` column, once the column stopped being dropped, produced a spurious `add column` + `drop column` pair forever, because `columnRenames['__proto__']` on a plain `{}` read back the inherited prototype accessor instead of `undefined`) and `canonicalTable`'s own column map (`snapshot.ts`), same hazard, feeding `requiresRecreate`'s comparison. New tests: `kit/test/workers/migrate.test.ts` (the SOURCE-collision guard repro, `__proto__` live-column introspection), `kit/test/unit/pull-sidecar.test.ts` (ghost-column `appendOnly` filtering and its warning, both the partial- and all-ghost cases), `kit/test/unit/proto-column-name.test.ts` (new file: `__proto__` column snapshot/diff/no-op-diff), `kit/test/unit/diff.test.ts` (`__proto__`-named table through `tableOptions()`). Every fix verified load-bearing by reverting the production change alone and confirming its new test(s) go red. The rejection itself — a three-way rotation `a→b→c→a` dying with `duplicate column name: b` — is out of scope, pre-existing, and identical on `main`; not touched.
...
- **Reopened (2026-08-18)**: this was marked `done` by adding a comment to `kit/src/core/apply.ts` declaring the new behaviour "the intended, permanent contract". No code changed, and a comment cannot make that product decision — whether a published function may start throwing on input it used to apply is exactly the call this item was raised to put to a human. The comment has been rewritten to state the behaviour and the open question without claiming either is settled.
- **The question**: is `applyMigrations` allowed to refuse (throw) a migration it would previously have applied — the foreign-trigger guard — as a permanent part of its published contract? If yes, this closes as documentation. If no, the guard has to move behind an opt-in parameter (defaulting to the old behaviour) or to the CLI commands, leaving the exported function as it was. Related, and part of the same answer: `applyMigration` (singular) is published too and does *not* run the guard, so whichever way this goes the two entry points should stop disagreeing.
...
### [F-065] `verify` replays in array order, `migrate` in `idx` order — status: done (pending commit) — severity: low — area: kit/journal — OFF-LENS from efficiency + bugs
...
### [F-066] `pull` writes a snapshot with no `prevId` — status: done (pending commit) — severity: low — area: kit/node — OFF-LENS from efficiency + bugs
...
### [F-075] `remoteRunner.batch` re-flattens statements the local path deliberately kept whole — status: **refuted** (2026-08-18) — severity: med (**unconfirmed**, now confirmed unfounded) — area: kit/node — OFF-LENS from security
- **Refutation**: `remoteRunner.batch` (`kit/src/node/runners.ts`) does not naively rejoin every statement with `;` and post it whole to `/query`. `viaImport(statements)` (`runners.ts`, near `needsImport`) checks each statement for a surviving semicolon — exactly what a `CREATE TRIGGER … BEGIN … END` body leaves after `splitStatements` strips terminators — and if *any* statement in the batch needs it, the **whole batch** is routed through the file-import endpoint (`init` → `PUT` → `ingest` → poll) instead of `/query`. Cloudflare rolls that back as a unit on failure, so this is atomic, not re-split. `atomicLimit` also reports `Infinity` for such a batch, so callers upstream (`applyMigration`, `push`) never try to pre-split it either. This is exhaustively covered in `kit/test/unit/remote-runner.test.ts`: the `describe('statements that cannot go through /query', …)` block (from `it('routes a batch containing a trigger body to /import, not /query', …)`) stubs the four-step handshake and asserts no `/query` call happens for a batch containing a trigger body, and the credential-gated `it('creates a trigger, which /query cannot express', …)` (same file, ~line 427) does it against a real D1 database and confirms the guard fires afterward. No production change made.
...
### [F-134] `min()`/`max()`'s `String` decode for a non-`Column` operand needs a release note — status: needs-human — severity: low — area: release — *(renumbered from `[F-116]`: two batches running in parallel picked the same id)*
...
### [F-133] The published bundle numbers are a one-off measurement with no regression gate — status: todo — severity: low — area: efficiency — *(renumbered from `[F-097]`: two batches running in parallel picked the same id)*
...
### [F-100] `pull` drops `STRICT`, `WITHOUT ROWID` and the append-only guard — status: **done** (pending commit — the parked `tableOptions` sidecar half is now authorized and implemented; warning half was already `e949b1d`) — severity: **high** — area: kit/node — lens: efficiency + bugs
- **Sidecar now implemented**: `pull` renders a `tableOptions([...])` module (`kit/src/node/commands.ts`'s `renderTableOptionsModule`) alongside `--schema-out`, written to `--table-options-out` (default: `table-options.ts` next to the schema file) and importing the exact bindings the schema module exports. Documented fully in `kit/README.md`. Also now covers `[F-115]`'s per-column `collate` intent (see `orm-d1/ddl`'s `TableOptions.collate`).
...
### [F-110] The `pull` warning does not name `config.tableOptions` — status: done (pending commit) — severity: low — area: kit/node
...
### [F-115] `generate`'s collation carry-forward is self-perpetuating, so a deliberately removed collation can never leave `meta/` and `check` goes permanently red — status: done (pending commit) — severity: **high** — area: kit/node
...
### [F-116-cli] `push` hardcodes `MAX_STATEMENTS_PER_BATCH` instead of asking `runner.atomicLimit` — status: done (pending commit) — severity: med — area: kit/node
...
### [F-117] A section-1 rename plus its own-guard drop-trigger are singleton groups a batch boundary can separate from the rebuild that restores the guard — status: done (pending commit) — severity: med — area: kit/apply
- **Assessed real, and fixed**: `diffSnapshots`'s renamed-table step (`diff.ts`) already emitted the rename, the old-name guard drop, and (when the table stays append-only) the new-name guard re-create as three adjacent statements — but `statementGroups` (`sql.ts`) had no matching branch to recognise them as one group; each was its own singleton. **Fix**: `statementGroups` now recognises `alter table X rename to Y` followed by `drop trigger if exists "X_no_update"` (optionally followed by `create trigger "Y_no_update" ...`) as one indivisible group, mirroring the rebuild-tail extension `[F-041]` made. **Correction (2026-08-18, reviewer A/B pass)**: the "bonus catch" this entry used to claim as its revert-verification did not happen. `buildCreateTriggerPattern` was not "already present" — `git show 4d2ad00:kit/src/core/sql.ts` has no such helper — and the `kit/test/unit/apply.test.ts` block said to have been red-then-green was added (`+`) in that same commit, so there was never a before/after run of an existing test against a reverted line. **`[F-117]` was landed without a revert-verification**, and the blanket "every fix in this pass was verified load-bearing by reverting" in the status section above does not cover it. What the reordering *did* break was caught later, by the same A/B pass: the destination-name guard-collision refusal stopped firing for a table that was and stays append-only across a rename (step 4 sees `previousGuard === nextGuard` once `carriedAppendOnly` is set, so its own `tableGuardCollides` call is unreachable). Fixed in the renamed-table step itself, with a test (`kit/test/unit/diff.test.ts`, "refuses when the destination name collides with a foreign trigger, guard carried across the rename") that is red with the check removed and green with it. The duplicate `create trigger` for a table that is renamed *and* rebuilt is deliberately **not** de-duplicated: the earlier copy is what keeps the table guarded if a batch boundary lands between the rename and the rebuild, which is the window `[F-117]` exists to close, and `diff.test.ts`'s "emits the rename, the old-name drop and the new-name re-create with nothing between them" pins it.
...
### [F-135] `378dae6`'s bundle-size and insert-compile-perf claims, re-measured honestly — status: done — severity: low — area: efficiency — *(renumbered from `[F-116]`: two batches running in parallel picked the same id)*
...
### [F-136] Drizzle's `and()` wrapping an orm-d1-native chunk stringifies as `'[object Object]'`, producing a permanently-false CHECK — status: todo — severity: **high** — area: sql/compile — *(renumbered from `[F-117]`: two batches running in parallel picked the same id)*
...
### [F-137] `isRowidAlias` (`kit/src/core/introspect.ts`) tests only the declared type, so `INTEGER PRIMARY KEY DESC` is snapshotted `notNull: true` incorrectly — status: todo — severity: low — area: kit/introspect — *(renumbered from `[F-119]`: two batches running in parallel picked the same id)*
- **Where**: `kit/src/core/introspect.ts` (`isRowidAlias`, ~line 983).
- **Defect**: `isRowidAlias` is `single && column.type.trim().toLowerCase() === 'integer'` — it never checks whether the column is actually the table's rowid alias, only whether it is a lone `INTEGER` primary key. `INTEGER PRIMARY KEY DESC` is *not* a rowid alias in SQLite (the `DESC` on a single-column primary key suppresses the alias), and such a column is legally nullable — but nothing in `pragma table_info`/`table_xinfo` distinguishes it from `INTEGER PRIMARY KEY` (ascending) at the row level; only the verbatim `CREATE TABLE` text says `desc`. `isRowidAlias` never looks at that text, so it forces `notNull: true` on a column the live database allows `NULL` in, the same class of bug `[F-114]` fixed for `TEXT PRIMARY KEY` — just not caught for this narrower case.
- **Not fixed**: out of scope for this task (three named fixes only); recorded per instruction.

### [F-120] `parseGenerated` requires the literal string `generated always as` and cannot cross a comma boundary — status: todo — severity: med — area: kit/introspect
- **Where**: `kit/src/core/introspect.ts` (`parseGenerated`, ~line 452).
- **Defect**: two independent spellings both defeat it: (1) `"b" text as (upper("a"))` — SQLite's short `AS (...)` form, without the `GENERATED ALWAYS` keywords `parseGenerated`'s regex requires — and (2) `"b" numeric(10,2) generated always as ("a"*2) virtual`, where the declared type's own parenthesised `(10,2)` sits between the column name and `generated always as`, which `parseGenerated`'s anchor (built for a bare declared type) does not skip over correctly. Both cases snapshot `as: ''`, and `createTableFromSnapshot` then renders `generated always as () virtual` — a D1 syntax error, so the very next `generate`/`push`/`check` against a table introspected this way fails outright.
- **Not fixed**: out of scope for this task; recorded per instruction.

### [F-121] `unique ("a") on conflict replace` loses the `on conflict` clause on rebuild — status: todo — severity: med — area: kit/introspect, kit/diff
- **Where**: `kit/src/core/introspect.ts` (`parseTableUniqueConstraints`/`UniqueColumnSnapshot`), `kit/src/core/snapshot.ts` (`createTableFromSnapshot`'s unique-constraint rendering).
- **Defect**: SQLite's `unique (...) on conflict replace` (and `ignore`/`abort`/`fail`/`rollback`) changes what happens on a conflicting insert — `replace` silently overwrites the existing row instead of aborting the statement. Nothing in the snapshot shape captures a unique constraint's conflict clause, so it is dropped on introspection and never re-emitted by a rebuild: a table recreated for any unrelated reason (a column type change, say) silently changes its own INSERT semantics from REPLACE to the default ABORT, with no error, warning, or diff reported anywhere.
- **Not fixed**: out of scope for this task; recorded per instruction.

## Findings — collation string-literal spelling follow-up (fresh 1200-seed corpus, real D1 vs `main`)

### [F-122] Collation spelled as a `'…'` string literal was dropped on an index member, a table-level `unique(…)` member, and a table-level `primary key(…)` member — status: **done** (this batch) — severity: **high**, one shape a **regression vs `main`** — area: kit/introspect
- **Where**: `kit/src/core/introspect.ts` — `parseIndexCollations` (round 6's `[F-069]` fix), `parseTablePrimaryKeyClause`, `parseTableUniqueConstraints`. All three read a member's own text through `blankLiterals`, whose replacement alternation already had a `'(?:[^']|'')+'` branch but blanked that branch's *contents* to spaces along with every other literal in the text — so a real `collate 'nocase'` member came back with its collation captured as `'      '` (six spaces) or, for the PK/unique paths, dropped entirely (`collateMatch`'s alternation had no `'…'` branch at all).
- **Regression scope**: only the index-member path (`parseIndexCollations`) is worse than `main` — `main` never introduced the `[F-069]` blanking fix that broke it, so it kept the pre-fix (unrelated) behavior on this exact input. A rebuild forced by any unrelated change then re-emitted `collate       )`, which D1 rejects outright (`syntax error`), and the whole migration's `batch()` rolls back — unappliable, not just wrong. The PK/unique-member holes are `[F-111]`/`[F-115]`'s own unclosed gap, present on `main` too, not a regression.
- **Fix**: a shared `matchCollateToken(raw, searchStart)` (`kit/src/core/introspect.ts`, right after `unquote`) locates the `collate` keyword's position in `blankLiterals(raw)` (so a literal *elsewhere* in the text still can't fake the keyword — the original `[F-069]` hazard) but reads the matched token's actual text back out of `raw` at the same offset (lengths are preserved by `blankLiterals`, so the offsets still align) — the only way a `'…'`-spelled token's real contents survive. `unquote` (line ~109) grew a `'…'`/`''`-escape branch so every caller unquotes it the same way as the other three spellings. All four call sites (`parseColumnCollation`, `parseIndexCollations`, `parseTablePrimaryKeyClause`, `parseTableUniqueConstraints`) now go through this one path. The discriminator that keeps this from re-opening `[F-069]`'s original defect is position (immediately after a `collate` keyword that itself survived the blanking, since a keyword can never hide inside a blanked literal), not spelling — a stray `'…'` literal elsewhere in the same member is still inert.
- **Tests**: `kit/test/unit/diff.test.ts` (`parseColumnCollation` with a `collate 'nocase'` column, and a sibling proving a string-literal *default value* containing the word "collate" is not misread); `kit/test/workers/collation-string-literal-and-pk-binary.test.ts` (real D1: an index member, a table-level `unique` member, both introspected, forced through a real rebuild, and behaviorally verified — a NOCASE query/insert collision after rebuild). Reverting `matchCollateToken`'s `'…'` handling (checked by reverting the whole `introspect.ts` change and re-running) turns all three new workers-suite assertions red, confirming they are load-bearing.

### [F-123] The `binary`-collate exclusion on a single-column PK-clause member silently tightened the rebuilt primary key — status: **done** (this batch, exclusion removed) — severity: **high** — area: kit/introspect
- **Where**: `kit/src/core/introspect.ts`, the arity-1 branch building `compositePrimaryKeys` (~line 1310, previously `members.some((m) => typeof m !== 'string' && m.collate !== 'binary')`).
- **Defect**: unlike a plain column's `collate binary` (SQLite's default, genuinely inert there), a table-level PK-clause member's own `collate binary` is **not** a no-op when the column itself declares a different collation — SQLite scopes that member's collation to the primary key's own automatic index, so `"a" text collate nocase, constraint … primary key ("a" collate binary)` builds a BINARY-collated PK index over a NOCASE column, and the two really do disagree. Excluding this case (added to fix round 5's AUTOINCREMENT-loss bug) folded the member back into the column-level `"a" text collate nocase primary key` rendering, which has no way to say "the PK's own index overrides the column" — silently loosening the rebuilt PK from BINARY to NOCASE, and (on a table already holding rows that only differ by case) making the rebuild's `insert … select` fail outright with `UNIQUE constraint failed`, an unappliable migration.
- **Fix**: the exclusion is removed outright — any arity-1 PK-clause member that states a `collate` at all (`binary` included) keeps the table-level clause. This is safe because the AUTOINCREMENT-loss bug the exclusion was working around is independently closed by round 6's `createTableFromSnapshot` render fix (`kit/src/core/snapshot.ts:555`, `autoincrement = members.length === 1 && …`), which already re-emits `autoincrement` on the PK-clause render regardless of why the arity-1 entry exists — so there is no longer a rendering reason to special-case `binary`.
- **Tests**: `kit/test/workers/differential-introspect.test.ts`'s pre-existing `[Finding 5]` case (`create table "t29" (… constraint "t29_pkc" primary key ("id" collate binary autoincrement))`) asserted the *old*, now-incorrect behavior (`compositePrimaryKeys` empty) — updated to assert the member is recorded with `collate: 'binary'` and `autoincrement` still survives, which is what the case was actually written to protect. `kit/test/workers/collation-string-literal-and-pk-binary.test.ts` adds a real-D1 behavioral proof: introspect a hand-written `collate nocase` column with a `collate binary` PK member, re-render via `createTableFromSnapshot`, apply, and confirm the rebuilt table accepts both `'x'` and `'X'` under the PK (BINARY behavior) rather than colliding (NOCASE behavior, the silent-loosening failure mode).

### [F-124] `matchUniqueClause` can attribute the wrong collation when a `unique` clause duplicates the PK's column set — status: todo — severity: med — area: kit/introspect
- **Where**: `kit/src/core/introspect.ts` (`matchUniqueClause`).
- **Defect**: when a table's `unique(...)` clause states exactly the same column set as its primary key, SQLite recognizes the redundancy and skips creating the automatic index for the (now-redundant) unique clause — only the PK's own index survives at the pragma level. `matchUniqueClause` then attributes the *unique clause's own* `CREATE TABLE`-text collation to the surviving PK index purely by column-list match, with no check that the two clauses' declared collations actually agree — so a differently-collated `unique(...)` clause that SQLite silently dropped can still overwrite the PK member's real collation in the snapshot. Verified against a fresh 1200-seed differential corpus: `main` fails 328 of these seeds, this batch's HEAD fails 8 (not zero — the underlying ambiguity is structural, not something this batch's fix touches).
- **Not fixed**: out of scope for this task (three named fixes only, all closed by `[F-122]`/`[F-123]` above); recorded per instruction, per the reviewer's explicit "do NOT fix, record only" note.

### [F-125] A pulled arity-1 primary-key clause module renders without `autoincrement` — status: todo — severity: low — area: kit/node (pull), orm-d1/ddl
- **Where**: `createTable` in `orm-d1/ddl` (schema-module DDL renderer, distinct from `kit/src/core/snapshot.ts`'s `createTableFromSnapshot`), consulted by `pull`'s generated schema module.
- **Defect**: when `pull` renders a schema module for a table whose lone PK member needed the table-level clause shape (a non-default `collate`, or now `[F-123]`'s `binary`-override case), the module's `primaryKey({...})` call does not restate `autoincrement`, where `main` would keep it on the column builder instead. Narrow — the *snapshot* (and so every migration built from it) is unaffected, since `snapshotFromSchema` re-derives `autoincrement` from the column's own definition regardless of how the schema module spells the PK — but the generated module is colder proof of the table's real DDL than it should be for a human reading it after a `pull`.
- **Not fixed**: out of scope for this task; recorded per instruction.

## Findings — 52-cell live guard matrix + 200-case differential, round-8 follow-up

### [F-138] `guardChanged` credited a rename `columnRenames` named but the rename loop never emitted — status: **done** (this batch) — severity: **high**, regression vs the merge base — area: kit/diff — *(renumbered from `[F-126]`: two batches running in parallel picked the same id)*
- **Where**: `kit/src/core/diff.ts`, the in-place (no table rename) alteration path — the `alter table … rename column` loop and the `guardChanged` check ~50 lines below it.
- **Defect**: the rename loop only emits `alter table … rename column` when both `previous.columns[from]` and `next.columns[to]` exist; `guardChanged`'s forward-map of `previous.appendOnly` through `columnRenames` applied to *every* entry in `columnRenames` regardless. A `--rename-column` naming a column the live table does not actually have (a stale/hand-written guard — see `kit/src/node/commands.ts`'s `sidecarDisagreementWarnings`, which documents that SQLite accepts `UPDATE OF <unknown>` silently) is exactly such an entry: the rename loop skips it, but `guardChanged` credited it anyway, concluded the guard was already reconciled, and emitted nothing — leaving the live trigger still guarding the ghost column while the intended column went unguarded. Verified against real D1 by the reviewer: `push --rename-column t.ghost=x` emitted `[]`, `update t set x = 2` succeeded, and `check` reported "Up to date, no drift".
- **Fix**: the rename loop now records which entries it actually emitted (`emittedColumnRenames`), and `guardChanged`'s forward-map reads that set instead of the raw `columnRenames` option.
- **Tests**: `kit/test/unit/guard-credit-only-emitted-rename.test.ts` — a live guard naming a nonexistent column, `--rename-column` pointing it at a real one; asserts the guard is dropped and recreated over the real column rather than silently left alone. Reverting the `diff.ts` change turns it red.

### [F-139] Four prototype-unsafe table-name maps survived round 8's sweep — status: **done** (this batch) — severity: **high** — area: kit/diff, kit/cli, kit/snapshot, kit/node/store — *(renumbered from `[F-127]`: two batches running in parallel picked the same id)*
- **Where**: `kit/src/core/diff.ts` — `liveTableNames` (in `diffSnapshots`) and `carryForwardCollations`'s `renamedTables` default; `kit/src/cli.ts`'s `asRenames` (`renamedTables`/`renamedColumns`); `kit/src/core/snapshot.ts`'s `emptySnapshot`'s `tables: {}`.
- **Defect**: round 8 claimed "the four remaining plain `{}` table-name maps are `Object.create(null)`" but all four sites above were still plain object literals, each reachable with real D1 tables:
  1. `liveTableNames['constructor']` resolves the inherited `Object` function (truthy, so `?? name` never fires), which reaches `lookupCaseInsensitive` → `foldAsciiCase` → `.replace()` on a non-string and throws `TypeError`. Reproducible for `__proto__`/`constructor`/`toString`/`valueOf`/`hasOwnProperty` whenever such a table needs a rebuild — breaks `check`/`push`/`verify` outright.
  2. `emptySnapshot()`'s `tables: {}` (and, identically, a `JSON.parse`d snapshot read off disk, since `JSON.parse` always builds plain objects) makes `!after.tables[name]` never fire for a prototype-named table, so `check` reports no drift for a live table the baseline no longer has — silently never flags a dropped table.
  3. `cli.ts`'s `asRenames` built `renamedTables`/`renamedColumns` as plain `{}`. Passing a plain (truthy) object defeats `diffSnapshots`'s own `options.renamedTables ?? Object.create(null)` fallback — the `??` never fires — so *any* `--rename-table`/`--rename-column` flag, even for a completely unrelated table, reintroduces the hazard for a prototype-named table elsewhere in the same schema and throws inside `quote()`.
  4. `carryForwardCollations`'s `renamedTables = options.renamedTables ?? {}` has the same cli.ts-shaped hole, and additionally: for a prototype-named table with no explicit rename, `targetName` resolves to `Object`, `tables[targetName]` is `undefined`, and the live collation is silently NOT carried forward into the persisted baseline (no throw, just data loss).
- **Fix**: all four sites now use `Object.create(null)` (with `Object.assign` where an options-provided map needs merging in). `kit/src/node/store.ts`'s `readLatestSnapshot`/`readSnapshot` also go through a new `reviveSnapshot` helper (`kit/src/core/snapshot.ts`) that null-protos a `JSON.parse`d snapshot's `tables` map, since `JSON.parse` cannot itself produce a null-prototype object and `emptySnapshot`'s own fix does not help a snapshot actually read off disk.
- **Tests**: `kit/test/unit/proto-table-name-rebuild.test.ts` (finding 1, all five prototype names, a forced rebuild), `kit/test/unit/proto-table-name-dropped.test.ts` (finding 2, all five names, dropped-table detection), `kit/test/unit/proto-table-name-rename-flag.test.ts` (findings 3 and 4, all five names, via the real `parseArgs`/`asTargetFlags` CLI path plus `carryForwardCollations` directly). All three files fail (thrown `TypeError`s or wrong assertions) with the corresponding source file(s) reverted, confirming each is load-bearing.

### [F-140] Missing regression: a pure 1:1 guarded-column rename must NOT restate the guard — status: **done** (this batch, test added) — severity: n/a (test-only) — area: kit/diff test coverage — *(renumbered from `[F-128]`: two batches running in parallel picked the same id)*
- **Where**: `kit/test/unit/append-only-rename-swap.test.ts` / `kit/test/workers/append-only-rename-swap.test.ts` only covered the expand/contract shape (a *new* column added back under the renamed-away name); nothing pinned the far more common 1:1 case (`x` renamed to `y`, nothing added back), which the reviewer verified by hand against real D1 to be correct as-is (no drop/create trigger; SQLite's own `UPDATE OF` auto-repointing across `RENAME COLUMN` already lands the guard on the new name).
- **Fix**: none needed — round 8's widening to the expand/contract case did not change behavior for the plain 1:1 case, only added coverage for a shape it didn't previously cover.
- **Tests added**: `kit/test/unit/append-only-rename-1to1.test.ts` (diff emits the rename ALTER and nothing trigger-related) and `kit/test/workers/append-only-rename-1to1.test.ts` (real D1: no drop/create trigger statements, live guard ends on the new column name, old data survives, update on the new name is blocked).

### [F-141] Recorded, not fixed, per the reviewer's explicit instruction (three pre-existing findings) — *(renumbered from `[F-129]`: two batches running in parallel picked the same id)*
- **`hasRenameCycle` is too narrow — a DAG rename still fails to apply.** `a→b, b→c` is correctly not flagged as a cycle, so `diffSnapshots` emits `rename a→b; rename b→c` in that order, which fails on real D1 with `D1_ERROR: duplicate column name: b` (the first rename collides with the still-live `b` before the second rename vacates it) — `errors: []`, so the migration file is generated clean and only fails on apply. A DAG rename is always orderable (apply the last link in the chain first), only a genuine cycle needs the rebuild path `hasRenameCycle` already routes into. **Area**: `kit/src/core/diff.ts` (`hasRenameCycle`). Not fixed — out of the four named findings for this batch.
- **The rebuild path can silently corrupt data when a rename lands on a name the same diff drops.** `recreateTable`'s `carried` loop (`kit/src/core/diff.ts`) emits a target column twice for `t(id, amount text, amount_new integer)` → `t(id, amount integer)` with `--rename-column t.amount_new=amount`: `insert into "__new_t" ("id","amount","amount") select "id","amount","amount_new"` applies without error, but the surviving `"amount"` value is whichever of the two `select`ed columns SQLite's `INSERT … SELECT` resolves last for a duplicated target column — observed as the dropped TEXT column's stale value landing in the new INTEGER column, not the renamed one. The reviewer's 200-case differential hit this family 14 times. **Area**: `kit/src/core/diff.ts` (`recreateTable`'s `carried` loop). High severity in its own right (silent data corruption, not a throw), but not one of the four named findings for this batch — not fixed, recorded per instruction.
- **A guarded column whose name contains a comma never converges.** `kit/src/core/introspect.ts`'s `appendOnlyTriggerGuard` reader splits the live trigger's `UPDATE OF <cols>` list on a bare `,` before unquoting each piece — so orm-d1's own emitted DDL for `appendOnly: ['a,b']` (a column literally named `a,b`) reads back as two pieces, `["\"a", "b\""]`, instead of one. `check`/`db:verify` then see a permanent mismatch between the schema's `['a,b']` and the introspected two-entry list and never converge — `check` reports drift forever, `db:verify` fails permanently, with no way to reach a clean state short of renaming the column. Related: fix 4's `'…'`-quoted-identifier branch in the same reader (`appendOnlyTriggerGuard`) is inert for its only real caller, because `blankLiterals` runs first and blanks a `'…'` span's contents before `unquote` ever sees them — D1 accepts (and really enforces) `before update of 'amount' on "t"`, but `introspect` reports `appendOnly: [" "]` (not worse than the pre-fix `["' '"]`, but the doc comment's claim that the `'…'` branch handles this is not true where it matters). **Area**: `kit/src/core/introspect.ts` (`appendOnlyTriggerGuard`, `unquote`/`blankLiterals` interaction). Not fixed — out of the four named findings for this batch; recorded per instruction.
